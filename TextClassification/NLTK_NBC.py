...
TextBlob has some problems for some non-ascii characters occuring in wikipedia articles. 
...
The methods here could definitely be improved, for example the 
training methods, which should take a category list instead of several arguments,
...
def createFile(category, numberOfArticles):
	...
	for cat in categoryList:
		ny = wikipedia.page(cat).content.encode('UTF-8')
...
To test the NLTK classifier, just call any of the test methods above with the classifier.
