...
def best_matching_category(text_to_test, words_to_check, uni_or_bi):
    """
    ...
    for category in CATEGORIES:
        ...
        if uni_or_bi == "uni":
            ...
            freq_dist_cat = word_statistics(category+"_catngram").most_common(words_to_check)
...
def sarah_create_bigram_from_noun(file):
    f = open(file)
    raw = f.read()
    tokenized_words = nltk.tokenize.word_tokenize(raw)
    pairs = ["".join(pair) for pair in nltk.bigrams(tokenized_words)]
    with open(file + "_bigram", "a+") as ngram_file:
        # Convert all of the items in lst to strings (for str.join)
        lst = map(str, pairs)
        # Join the items together with commas
        # line = " ".join(pairs) ????
...
'''
for cat in testList:
    sarah_create_bigram_from_noun(cat + '_nouns')
'''
...
def category_decider(file, words_to_check):
    if file.endswith("_nouns"):
        uni_or_bi = "uni"
    else:
        uni_or_bi = "bi"
    word_stat = word_statistics(file).most_common(words_to_check)
    best_cat_tuple = best_matching_category(word_stat, words_to_check, uni_or_bi)
    best_match = CATEGORIES[best_cat_tuple[0]]
    # return best_match
